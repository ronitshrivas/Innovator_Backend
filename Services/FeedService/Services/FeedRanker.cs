...
public class HeuristicFeedRanker : IFeedRanker
{
        ...
        int seed)
    {
        ...
        var scored = candidates.Select(p => new Scored(p, Score(p, now, topCats, rng))).ToList();
    ...
    private double Score(
        PostResponse p, DateTime now, HashSet<string> topCats, Random rng)
    {
        ...
        // Affinity: strong boost when the viewer follows the author.
        var affinity = p.IsFollowed ? 1.0 : 0.0;
